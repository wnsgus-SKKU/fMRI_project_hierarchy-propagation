...
if __name__ == '__main__':
    ...
    for li in np.arange(locs.shape[0]-1):
        tmp_prin = np.array([]) 
    ...
    for li in np.arange(seg_all.shape[1]):
        temp1 = seg_all[:,li]

        if np.sum(np.isnan(temp1.astype(np.float))) < seg_all.shape[0] * 0.2:
            if seg_all2.shape[0] == 0:
                seg_all2 = inpaintNans(temp1)
            else:
                seg_all2 = np.concatenate((seg_all2, inpaintNans(temp1)),axis=1)
    delay_test3 = seg_all2 - np.tile(np.mean(seg_all2),seg_all2.shape[1])
    
    delay_test3 = delay_test3.astype(np.float)
    delay_test3 = delay_test3[~np.isnan(delay_test3)]
    
    delay_test3_small = delay_test3[:10000, :10000]
