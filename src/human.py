...
if __name__ == '__main__':
    ...
    idx_tem_prin = np.zeros((size(tw1),size(locs)-1),object) # need to check range 

    for li in range(1,size(locs)): # need to check range 
        tmp_prin = np.array([])
        tmp_prin = tw1[:,locs[li]:locs[li+1]]

        for lj in range(1,size(tw1)+1):
    ...
    for ln in range(1,size(locs,1)): # need to check range 
        x_ax = np.array([])
        x_ax = np.arange(0,(sz(ln)-1)/size(idx_tem_prin,1),(sz(ln)-1))
    
        if np.sum(np.isnan(idx_tem_prin[:,ln])) > 14:
            rval_prin_2[ln] = np.NaN

        else:   
            rval_prin_2[ln] = np.corrcoef(x_ax.T,idx_tem_prin[:,ln])[0,1]
